...
/-- The same packing, built top-down (`hor seed k` = cells `256 - k, …, 255`) as a
direct `Nat.rec` loop, which the kernel runs much faster than the structural
recursion of `enc`. `hor_eq_enc` relates the two. -/
def hor (seed k : Nat) : Nat :=
  Nat.rec (motive := fun _ => Nat) 0
    (fun j acc => Nat.add (Nat.mul 2 acc) (cell seed (Nat.sub 255 j))) k

/-- The seeded initial row: cell 0 is on, cell 1 is off, the rest come from `cell`. -/
def initV (seed : Nat) : Nat :=
  Nat.add (Nat.mul 4 (hor seed 254)) 1
...
theorem hor_eq_enc (seed : Nat) : ∀ k, k ≤ 254 → hor seed k = enc seed (256 - k) k
  | 0, _ => rfl
  | k + 1, hk => by
    show 2 * hor seed k + cell seed (255 - k) = 2 * enc seed (256 - (k + 1) + 1) k + cell seed (256 - (k + 1))
    rw [hor_eq_enc seed k (by omega), show 256 - (k + 1) + 1 = 256 - k by omega,
      show 256 - (k + 1) = 255 - k by omega]

theorem initV_eq (seed : Nat) : initV seed = encodeRow (initRowFor seed) := by
  have h : initV seed = 4 * enc seed 2 254 + 1 := by
    show 4 * hor seed 254 + 1 = _
    rw [hor_eq_enc seed 254 (Nat.le_refl _)]
  rw [h]
