...
geometric sum. `hgeo_eq`, `rowF_eq` and `impl_eq` connect it to `geo` / `rowv`,
which the proofs use. -/

/-- Horner form of `geo`. -/
def hgeo (a : Nat) : Nat → Nat
  | 0 => 1
  | J + 1 => Nat.add 1 (Nat.mul a (hgeo a J))

def rowF (n X M : Nat) : Nat → Nat
  | 0 => 1
  | k + 1 => Nat.mod (Nat.mul (rowF n X M k)
      (hgeo (Nat.pow X (Nat.add k 1)) (Nat.div n (Nat.add k 1)))) M
